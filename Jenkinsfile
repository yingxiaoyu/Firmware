    ...
    stage('Analysis') {
      ...
      parallel {
        ...
        stage('Colcon build on ROS2 workspace') {
          agent {
            docker {
              image 'px4io/px4-dev-ros2-dashing:2019-12-18'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw -e HOME=$WORKSPACE'
            }
          }
          steps {
        ...
        stage('Style check') {
          agent {
            docker { image 'px4io/px4-dev-base-bionic:2019-12-18' }
          }
          steps {
        ...
        stage('px4_fmu-v2 (bloaty)') {
          agent {
            docker {
              image 'px4io/px4-dev-nuttx:2019-12-18'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw'
            }
          }
          steps {
        ...
        stage('SITL unit tests') {
          agent {
            docker {
              image 'px4io/px4-dev-base-bionic:2019-12-18'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw'
            }
          }
          steps {
        ...
        stage('Clang analyzer') {
          agent {
            docker {
              image 'px4io/px4-dev-clang:2019-12-18'
              args '-e CCACHE_BASEDIR=$WORKSPACE -v ${CCACHE_DIR}:${CCACHE_DIR}:rw'
            }
          }
          steps {
